...
%MACRO AGGR_FULL_WEEK_TO_MONTH();
  PROC SORT DATA=OUTPUT.TV_MM_GRP_BY_DMA_REPORT_WEEK OUT=REPORT_WEEK_WGRPS; 
    BY DMA FULL_WEEK REPORT_END;
  RUN;
  ...
  DATA FULL_WEEK_ADSTKS;
    ...
    KEEP DMA FULL_WEEK %SYSFUNC(TRANWRD(&ADSTK_COLS.,_XX_,_:));;
  RUN;
  ...
  */ 
  ...
  DATA ADS_REPORT_WEEK;
    MERGE REPORT_WEEK_WGRPS(IN=A) FULL_WEEK_ADSTKS(IN=B);
    BY DMA FULL_WEEK;
  RUN;
  DATA ADS_REPORT_WEEK;
    SET ADS_REPORT_WEEK;
  ...
  DATA ADS_BY_DMA_REPORT_WEEK; 
    ...
    YEARMO = put(REPORT_END,yymmn6.);
  RUN;
  PROC MEANS DATA=ADS_BY_DMA_REPORT_WEEK(DROP=REPORT_END FULL_WEEK) SUM NOPRINT NWAY;
    CLASS DMA YEARMO;
    VAR _NUMERIC_ ;
