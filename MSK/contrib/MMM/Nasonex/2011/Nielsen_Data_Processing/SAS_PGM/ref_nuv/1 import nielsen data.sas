...
*                                                                 ;
***************************************************************************;
TITLE "NUVARING NMR DATA IMPORT - 2007 THROUGH 2009";
...
* NATL TV - GRPS;
%NGETDATA(Nuvaring National Revised,data,A20,P2281,NATLTV_GRPS);

* NATL TV - SPEND;
%NGETDATA(NuvaRing National TV $ Jan 07 - Dec 09, data, A6, I2551, NATLTV_SPEND);

* NATL MULTIMEDIA;
%NGETDATA(NUVARING NATIONAL MULTI $ JAN 07 - DEC 09,DATA,A6,M434,NATL_MM);


* LOCL TV - GRPS;
%NGETDATA(Nuvaring Local Revised,data,A20,L61992,LOCLTV_GRPS);

* LOCL TV - SPEND;
%NGETDATA(NUVARING LOCAL TV $ JAN 07 - DEC 09,DATA,A6,G2826,LOCLTV_SPEND);

* LOCL MULTIMEDIA;
%NGETDATA(NUVARING LOCAL MULTI $ JAN 07 - DEC 09,DATA,A6, N15,LOCL_MM);
...
* SET ALL LOCL TV GRP/IMP DATA TOGETHER;
PROC SORT DATA=LOCLTV_GRPS; 
	BY MARKET BRAND REPORT_PERIOD TV_DAYPART COMMERCIAL_DURATION CREATIVE; RUN;
PROC SORT DATA=LOCLTV_SPEND; 
	BY MARKET BRAND REPORT_PERIOD TV_DAYPART COMMERCIAL_DURATION CREATIVE; RUN;

DATA PRINT_GRPS;SET DTC.PRINT_GRP;LONG_BRAND=BRAND;RUN;
...
PROC SORT DATA=DTC.PRINT_GRP; BY MARKET BRAND REPORT_PERIOD; RUN;
...
DATA LOCL_P;
   SET LOCL DTC.PRINT_GRP(IN=P); 
   BY MARKET BRAND REPORT_PERIOD;
   IF P THEN RECTYPE='PR'; 
   IF RECTYPE='PR' THEN DO; DAYPART="NA"; COMMERCIAL_DURATION="NA"; CREATIVE="NA";
   END;
RUN;
 
PROC FREQ DATA=LOCL_P; TABLES RECTYPE/LIST MISSING; RUN;


TITLE2 "LIST CONTENTS OF LOCAL TV AND MULTIMEDIA DATASET"; RUN;
PROC CONTENTS DATA=LOCL_p VARNUM; RUN;
TITLE2 ""; RUN;

PROC FREQ DATA=NATL; TABLES COMMERCIAL_DURATION/LIST MISSING; RUN;
PROC FREQ DATA=LOCL_P; TABLES COMMERCIAL_DURATION/LIST MISSING; RUN;

* MAKE NATL AND LOCL DATASETS PERMANENT;
DATA DTC.NATL_TV_MM_JAN07_DEC09; SET NATL; YEAR=YEAR(REPORT_PERIOD); MONTH=MONTH(REPORT_PERIOD); 
ARRAY NUMS(*) _NUMERIC_; DO K=1 TO DIM(NUMS); IF NUMS(K)=. THEN NUMS(K)=0; END; 
 IF UPCASE(COMMERCIAL_DURATION)='10 SECONDS' THEN DURATION=10; ELSE
 IF UPCASE(COMMERCIAL_DURATION)='15 SECONDS' THEN DURATION=15; ELSE
 IF UPCASE(COMMERCIAL_DURATION)='30 SECONDS' THEN DURATION=30; ELSE
 IF UPCASE(COMMERCIAL_DURATION)='35 SECONDS' THEN DURATION=35; ELSE
 IF UPCASE(COMMERCIAL_DURATION)='50 SECONDS' THEN DURATION=50; ELSE
 IF UPCASE(COMMERCIAL_DURATION)='60 SECONDS' THEN DURATION=60; RUN;

PROC FREQ DATA=DTC.NATL_TV_MM_JAN07_DEC09; TABLES COMMERCIAL_DURATION*DURATION/LIST MISSING; RUN; 

DATA DTC.LOCL_TV_MM_JAN07_DEC09; SET LOCL_P; YEAR=YEAR(REPORT_PERIOD); MONTH=MONTH(REPORT_PERIOD); 
ARRAY NUMS(*) _NUMERIC_; DO K=1 TO DIM(NUMS); IF NUMS(K)=. THEN NUMS(K)=0; END; 
 IF UPCASE(COMMERCIAL_DURATION)='15 SECONDS' THEN DURATION=15; ELSE
 IF UPCASE(COMMERCIAL_DURATION)='30 SECONDS' THEN DURATION=30; ELSE
 IF UPCASE(COMMERCIAL_DURATION)='45 SECONDS' THEN DURATION=45; ELSE
 IF UPCASE(COMMERCIAL_DURATION)='60 SECONDS' THEN DURATION=60; 
RUN;
 PROC FREQ DATA=DTC.LOCL_TV_MM_JAN07_DEC09; TABLES COMMERCIAL_DURATION*DURATION/LIST MISSING; RUN; 

PROC FREQ DATA=DTC.NATL_TV_MM_JAN07_DEC09; TABLES BRAND*CREATIVE*YEAR*MONTH/LIST MISSING ; RUN;
PROC FREQ DATA=DTC.LOCL_TV_MM_JAN07_DEC09; TABLES BRAND*CREATIVE*YEAR*MONTH/OUT=FREQCNT LIST MISSING; RUN;
PROC FREQ DATA=DTC.NATL_TV_MM_JAN07_DEC09; TABLES DURATION/LIST MISSING ; RUN;
PROC FREQ DATA=DTC.LOCL_TV_MM_JAN07_DEC09; TABLES DURATION/LIST MISSING; RUN;
