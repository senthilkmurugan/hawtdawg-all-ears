PROC PRINTTO LOG="\\wpushh01\dinfopln\PRA\DTC\NuvaRing\2011 profit plan\SAS Logs and Lists\13a prep GRP adstocks(no lag).LOG" NEW
			PRINT="\\wpushh01\dinfopln\PRA\DTC\NuvaRing\2011 profit plan\SAS Logs and Lists\13a prep GRP adstocks(no lag).LST" NEW;
***************************************************************************;
...
/* 
Copy permanent dataset to a local temp dataset and use this to process to save exec time 
*/
data weekly_local_weighted_grps_jun10;
  set DTC1.weekly_local_weighted_grps_jun10;
...
/* 
Move proc sort from within the macro to outside, to avoid multiple time consuming sorts.
*/
PROC SORT DATA=weekly_local_weighted_grps_jun10 OUT=TWEEKS3; BY MONYY WEEK; RUN;



%MACRO WEIGHT (PROD,AGE,CURRENT,NAME); 

LIBNAME &PROD "\\WPUSHH01\DINFOPLN\PRA\DTC\NUVARING\2011 Profit Plan\SAS Datasets\&PROD\DECAY";


* IDENTIFY HIGHEST WEEK NUMBER IN CURRENT MONTH;
/*PROC SORT DATA=weekly_local_weighted_grps_jun10 OUT=TWEEKS3; BY MONYY WEEK; RUN;*/
DATA _NULL_; SET TWEEKS3;BY MONYY;MONTHLABEL=PUT((MDY(MONTH,1,YEAR)),MONYY5.); IF LAST.MONYY AND 
...
DATA &PROD.SUM4 (KEEP=MARKET REPORT_PERIOD MONTH WEEK NWEEK RELWEEK
        ...
        SET  weekly_local_weighted_grps_jun10(WHERE=(WEEK<=&CWEEK) IN=W);
...
%WEIGHT(NUVA,F1849,('JAN10'),JAN10);
%WEIGHT(NUVA,F1849,('FEB10'),FEB10);
%WEIGHT(NUVA,F1849,('MAR10'),MAR10);
%WEIGHT(NUVA,F1849,('APR10'),APR10);
%WEIGHT(NUVA,F1849,('MAY10'),MAY10);
%WEIGHT(NUVA,F1849,('JUN10'),JUN10);
