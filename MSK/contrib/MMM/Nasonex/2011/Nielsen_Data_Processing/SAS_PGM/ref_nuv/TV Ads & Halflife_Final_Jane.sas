...
options nosymbolgen nomprint mlogic;  
...
%LET MAX_WEEK=16; /* number of weeks to define the longest considered half-life */
...
ods html file="TV Ads & Halflife_Final.xls" PATH=ODSOUT;
...
data DECAY_WEIGHTS;
	...
	overall=1;
	array x(*) X1-X10000;
	X(1)=round((decay)**1,0.0001);
	do i= 2 to 10000 while (X(i-1) >=0.0001);
		X(i)=round((decay)**i,0.0001);
		overall=i;
	end;
	if last then do;
		call symput('LL',overall+2);
	end;
	%put LL=&LL;
run;
...
data &out1.(KEEP=X0-X%sysfunc(left(&LL.)));
	LENGTH X0 3.;
	set DECAY_WEIGHTS;
...
*%MAIN(8,step7,step8,DECAY_8_WEEK_WEIGHTS,DECAY_8_WEEK_SCENARIO,EIGHT);
%MAIN(16,step15,step16,DECAY_16_WEEK_WEIGHTS,DECAY_16_WEEK_SCENARIO,SIXTEEN);
