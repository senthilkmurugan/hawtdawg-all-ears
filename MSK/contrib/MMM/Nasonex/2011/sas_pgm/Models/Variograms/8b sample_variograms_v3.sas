...
*/
/* 
This code has variograms for the selected model after exploring the half life's.
   objective is to see if the models could be improved by choosing an error covaiance structure for repeated measures. 
*/
/* 
This code is executed after outlier detection and removal by influence diagnostics.
   objective is to see if the outlier removed models could be improved by choosing an error covaiance structure for repeated measures. 
*/
...
run;
data mdl1_no_outliers;
  set mdl1;
  if outlier_dma_ind = 1 then delete;
run;
proc sort data=mdl1_no_outliers; by dma yearmo; run;
...
%LET PATH=C:\Documents and Settings\smuruga2\PROJECTS\DTC\Nasonex\2011\Data\MODEL_DATA\Explore_1\Variograms\SelectedModel_2; *LOCATION FOR SAMPLE VARIOGRAM AND AUTOCORR PLOTS;
%LET MDLDSET=mdl1_no_outliers;
...
%MACRO ExcelExport(indata,sheet_nm);
  PROC EXPORT DATA=&indata. OUTFILE="&PATH.\resid_corrs_2.XLS"
	  DBMS=EXCEL REPLACE;
	  SHEET="&sheet_nm";
  RUN;
%MEND ExcelExport;
...
data mdl1_2;
  ...
  rtime = time_idx - 6;
run;
proc mixed data=mdl1_2(where=(outlier_dma_ind=0)); 
   class dma rtime;
...
proc mixed data=mdl1_2(where=(outlier_dma_ind=0)); 
   class dma rtime;
   model pnasnrx = t1 - t24 mc_nrp lag_pnasnrx pothnrxsh
                   detmd_40 sammd_50 grpomn_70 grpotc_60 grpnas_50*semS1 grpnas_50*semS2 / outp=res_outp_3 outpm=res_outpm_3 s;
   RANDOM intercept / subject=dma;
   *REPEATED rtime/ TYPE=SP(EXP)(RTIME) SUBJECT=dma rcorr;
run;
